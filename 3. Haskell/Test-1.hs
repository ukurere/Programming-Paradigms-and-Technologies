-- Test 1 (easy): w = "0", automaton is already in an accepting state
...
main = do
  ...
  putStrLn "Test 1 (easy): w = \"0\" -- after reading w the automaton is in state 0 (accepting)"
  let w = "0"
  case canExtend fa w of
    Just wy -> do
      putStrLn $ "Result:   Yes, example word wy = \"" ++ wy ++ "\""
      putStrLn   "Expected: Yes, wy = \"0\"  (0 is divisible by 3)"
    Nothing ->
      putStrLn   "Result:   No"
