-- Test 3 (hard): w = "1" -- automaton is in state 1 (remainder 1),
-- BFS must find the shortest y so that w++y is accepted.
-- From state 1: reading '1' -> state 0 (accepting), so y = "1", wy = "11" (= 3)
...
main = do
  ...
  putStrLn "Test 3 (hard): w = \"1\" -- state 1 (remainder 1), BFS finds y"
  let w = "1"
