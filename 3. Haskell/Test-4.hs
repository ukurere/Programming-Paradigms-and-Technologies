-- Test 4 (hard): w = "10" -- automaton ends in state 2 (remainder 2),
-- BFS explores two levels to find the shortest y.
-- From state 2: '0'->1, '1'->2  (no accepting state yet)
-- From state 1: '1'->0 (accepting!)
-- So y = "01", wy = "1001" (binary 9 = 3*3, divisible by 3)
...
main = do
  ...
  putStrLn "Test 4 (hard): w = \"10\" -- state 2 (remainder 2), BFS needs 2 steps"
  let w = "10"
  case canExtend fa w of
    Just wy -> do
      putStrLn $ "Result:   Yes, example word wy = \"" ++ wy ++ "\""
      putStrLn   "Expected: Yes, wy = \"1001\" (binary 1001 = 9, divisible by 3)"
    Nothing ->
      putStrLn   "Result:   No"
